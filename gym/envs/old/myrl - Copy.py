...
'''
import gym, random, time, shutil, os
...
class MyrlEnv(gym.Env):
    ...
    def __init__(self):
        ...
        else:    filen ='EWZ_1_Min.csv'
        
        if nt: pathfile = Path(r'C:\Users\Jake\py\Deep-RL-Keras')
        else: pathfile = Path(r'/root/Deep-RL-Keras')
        ...
        self.qt = self.scaler.fit_transform(read) * 2 - 1
        self.scaler1 = RobustScaler() # copy=False) #sine:2.2
        self.rs = self.scaler1.fit_transform(read)
        ...
        if sine: # benchmark settings for sine
            ...
            self.mul = 8 #7
            eps = 110 #65 #50
            self.cycle = int(1.5 * eps)
            # self.cycle = 1

            # 0:pkmd 1:pkwd 2:cxd 3:cxmd 4:Close 5:pkw 6:pkm
            self.data = self.qt[:,3] #* 2
            self.prevbars = 1 #int(eps/2)
            
            self.g1 = self.qt[:,0]
            self.prevbars1 = 3 #int(2 * eps)
            
            self.g2 = self.qt[:,0]
            self.prevbars2 = 0 #int(eps/2)
        ...
        self.updatefile = 'r:\data.csv'
        self.old = os.path.getmtime(self.updatefile)
        self.X = 0

    def UpdateData(self):
        read = np.loadtxt(self.updatefile, skiprows=8 , delimiter=' ', dtype=np.float64)
        if self.maxseries +2 > read[:,0].size:
            print("\a\r\nAttention: data.csv size should be at least " + str(self.maxseries +2) + " lines" ) 
            exit()
        else:
            # read = read[-self.maxseries:,]  # Get last N lines
            self.qt = self.scaler.fit_transform(read) * 2 - 1
            self.rs = self.scaler1.fit_transform(read)
            self.data = self.rs[-self.maxseries:,0]
            self.g1 = self.qt[-self.maxseries:,1]
    ...
    def step(self, action):     # **********************************************
        ...
        while True: # Pause for new data
            time.sleep(1)
            self.new = os.path.getmtime(self.updatefile)
