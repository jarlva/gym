...
class rlEnv(gym.Env):
    ...
    def __init__(self):
        ...
        if sine: # benchmark settings for sine
            ...
            self.g1 = qt[:,2]
            self.prevbars1 = int(2 * eps) #2 210 #252
            ...
            self.g1 = qt[:,0]
            self.prevbars1 = int(2 * eps) #250
            
            self.g2 = qt[:,0]
            self.prevbars2 = int(eps/2)
    ...
    def _get_ob(self):
        ...
        state = np.concatenate((data, g1, (self.bar, self.buybar, self.price))) # for some reason needs extra ()
        # g2 = self.g2[self.X - self.prevbars2+1:self.X+1] 
        # state = np.concatenate(( data, g1, g2, (self.bar, self.buybar, self.price))) # for some reason needs extra ()
