...
# class MyCartPoleEnv(gym.Env): 
    ...
    def __init__(self):
        ...
        qt = self.scaler.fit_transform(self.read) * 2 - 1
        ...
        if sine: # benchmark settings for sine
            ...
            self.datacol = 1 ; self.data = qt[:,self.datacol]  #* 2
            self.prevbars = int(eps * 2)
    ...
    def _get_ob(self):
        ...
        # assert state.size== 130, str(state.size) + " " + str(data.size) \
            # + " " + str(self.bar) + " " + str(self.price) + " " + str(self.X)
            # + " " + str(g1.size)
    ...
    def reset(self):
        ...
        # TS remarked for training speed        
        # if not RT: 
        self.X = random.randint(self.maxseries,  self.data.size-self.cycle - 2)
