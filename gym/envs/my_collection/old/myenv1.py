...
class MyEnv1(gym.Env, utils.EzPickle):
    metadata = {'render.modes': ['human', 'rgb_array']}

    def __init__(self, obs_type = 'image', frameskip = 3 , repeat_action_probability=0.0):
        ...
        self.viewer = None
        self.state = None
        self.frameskip = frameskip # originaly 4
        self.done = False
        ...
        image = Image.open(r"C:\Users\Jake\py\files\Sim\SPY.bmp").convert('L')
        #print (image.height, image.width )
        # initial crop
        box = (724,10,1823,326)
        self.im = image.crop(box) # intial cleanup
        # print ( im.width , im.height)
        scale = 5 # how much to scale down
        new_size = ( int( self.im.width/ scale), int(self.im.width/ scale)  ) # the width/height must be the same for thumbnail
        # new_size = (128, 128)
        self.im.thumbnail(new_size)
        self.win_width = 10 # width of sliding window
        self.y2 = self.im.height

        # setup plot      
        screen_height = self.im.height   #64 #200 #348 # 100 dots per inch
        screen_width = self.im.width     #64 #400 #678
    ...
    def render(self, mode='human'): # visualize=False skips this function entirely
    ...
    def step(self, a):
        ...
        for _ in range(self.frameskip): #(num_steps):
