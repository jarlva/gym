...
class MyCartPoleEnv(gym.Env): 
    ...
    def __init__(self):
        ...
        if sine: # benchmark settings for sine
            self.mul = 1 #0.5 #2.17 #5.4 #5.7 #5.4 #2.17
            eps = 64
            ...
            self.data = qt[:,1] #rs[:,0] #* 0.63 #* 2.17
            self.prevbars = int(2 * eps) #1 #int(2 * eps) #eps

            self.g1 = qt[:,1]
            self.prevbars1 = 0 # int(2 * eps) #210 #252
        ...
        # self.X = random.randint(self.maxseries, int(self.data.size-4000))
        ...
        self.X = 200
        # self.X = random.randint(self.maxseries, int(self.maxseries+500))
        # self.reset() already done in main.py

    def _get_ob(self):
        data = self.data[self.X - self.prevbars+1:self.X+1]
        # state = np.hstack(( data, self.bar, self.price))
    ...
    def reset(self):
        ...
        if not self.ok_to_render: # training
            self.X = random.randint(self.maxseries-1, int(self.data.size-self.cycle))
        else:
            # plot reset
